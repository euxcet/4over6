...
int ip_pool_find_fd(in_addr_t addr) {
	for(int i = 4; i < 255; i++) {
		if (client[i].addr.s_addr == addr) {
			return client[i].fd;
		}
	}
	return 0;
...
in_addr ip_pool_create_fd(int fd) {
	for(int i = 4; i < 255; i++)
		if (!client[i].status) {
			client[i].status = 1;
			client[i].fd = fd;
			return client[i].addr;
		}
}

int ip_pool_remove_fd(int fd) {
	for(int i = 4; i < 255; i++)
		if (client[i].fd == fd) {
			client[i].status = 0;
			client[i].fd = 0;
...
void process_tun() {
	...
	if (hdr -> version == 4) {
		...
		else if ((fd = ip_pool_find_fd(hdr -> daddr)) > 0) {
			msg.type = MSG_NET_RESPONSE;
			msg.length = n;
			write(fd, &msg, msg.length);
			return;
		}
	}
}
...
int process_client(struct epoll_event event) {
	...
	if (msg.type == MSG_IP_REQUEST) {
		...
		in_addr addr = ip_pool_create_fd(event.data.fd);
		
		msg.type = MSG_IP_RESPONSE;
		snprintf(msg.data, MAX_LENGTH, "%s 0.0.0.0 202.38.120.242 8.8.8.8 202.106.0.20 ", inet_ntoa(addr));
	...
	for(int i = 0; i < count; i++) {
		if (events[i].data.fd == server_fd) {
			...
			if (process_client(events[i]) < 0) {
				...
				ip_pool_remove_fd(events[i].data.fd);
...
int main() {
	...
	for(int i = 0; i < 256; i++) {
		char addr[100];
		sprintf(addr, "10.0.0.%d\n", i);
		client[i].addr.s_addr = inet_addr(addr);
		client[i].status = 0;
		client[i].fd = 0;
	}
	client[0].status = client[1].status = client[2].status = client[3].status = client[255].status = 1;
	...
	while (true) {
		...
		if (time(NULL) - heartbeat_ts > 20) {
			...
			for(int i = 4; i < 255; i++)
				if (client[i].status)
					write(client[i].fd, &msg, msg.length);
		}
	}
